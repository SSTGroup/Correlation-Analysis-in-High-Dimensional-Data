function [W,B]=mcca(X,numOfCV,costType)
...
function [W,B] = mcca_genvar(X,numOfCV)
...
%         theta(n) = prod(prod(R_hat*R_hat'));
%         theta(n) = prod(prod(R_hat));
        %% Check termination condition
        if sum(abs(theta(n) - theta_old)) < eps | n == numMaxIter
            theta_opt(s) = theta(n);
            break;
